...
import logica.InspeccionFito;

public class DaoInspFit {
    ...
    public int obtenerId() {
        ...
        String sql = "SELECT MAX(ID_INSPECCION_FITO) FROM INSPECCION_Fitosanitaria";
    ...
    public boolean guardarInspeccion(int siguienteId, InspeccionFito insp) {
        ...
        String sql = "INSERT INTO INSPECCION_fitosanitaria ("
                       + "ID_INSPECCION_FITO, ID_ORDEN, NRO_PLANTAS_EVALUADAS, ESTADO_FENOLOGICO, CANTIDAD_PLANTAS_INFESTADAS, PORCENTAJE_DE_INFESTACION, "
                       + "COMENTARIOS"
                       + ") VALUES (?, ?, ?, ?, ?, ?, ?)";
        ...
        try {
            ...
            double porcentaje = Double.parseDouble(insp.getPORCENTAJE_DE_INFESTACION());

            // 1. ID
            stmt.setInt(1, siguienteId);
            // Usamos los Getters del objeto InspeccionTecnica
            stmt.setString(2, insp.getIdOrden());
            stmt.setString(3, insp.getNRO_PLANTAS_EVALUADAS());
            stmt.setString(4, insp.getESTADO_FENOLOGICO());
            stmt.setString(5, insp.getCANTIDAD_PLANTAS_INFESTADAS());
            stmt.setDouble(6, porcentaje);
            stmt.setString(7, insp.getCOMENTARIOS());
